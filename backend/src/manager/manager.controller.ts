...
export class ManagerController {
  ...
  daily(@Query('date') date?: string) {
    return this.manager.dailyPayments(date || todayISO());
  }

  @Get('payments/monthly')
  monthly(@Query('year') year?: string, @Query('month') month?: string, @Query('date') date?: string) {
    const d = date || todayISO();
    return this.manager.monthlyPayments(Number(year || d.slice(0, 4)), Number(month || d.slice(5, 7)));
  }
  ...
  customersReport(@Query('from') from?: string, @Query('to') to?: string) {
    return this.manager.customersReport(from, to);
  }
  ...
  @Permissions('reports.view')
  reportMonthly(@Query('year') year?: string, @Query('month') month?: string, @Query('date') date?: string) {
    const d = date || todayISO();
    return this.manager.monthlyPayments(Number(year || d.slice(0, 4)), Number(month || d.slice(5, 7)));
  }
  ...
  reportDebts(@Query() query: PageQueryDto) {
    return this.manager.debts({ tab: query.tab, q: query.q, skip: 0, pageSize: 10000 });
  }

  @Get('reports/occupancy')
  @Permissions('reports.view')
  occupancy() {
    return this.manager.occupancy();
  }

  @Get('reports/check-history')
  @Permissions('reports.view')
  reportChecks(@Query('from') from?: string, @Query('to') to?: string) {
    return this.manager.checkHistory({ from, to, skip: 0, pageSize: 10000, tab: 'all' });
  }
