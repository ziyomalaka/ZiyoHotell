...
type Profile = {
  ...
  address?: string | null;
  debt: number;
  occupancy?: { stay: { room: { number: string }; bed: { number: number } } } | null;
  stays: { id: string; startDate: string; status: string; room: { number: string }; bed: { number: number }; totalAmount: number; paidAmount: number }[];
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="card p-5 text-sm">
        ...
        <p>Xona/o‘rin: {c.occupancy ? `${c.occupancy.stay.room.number}/${c.occupancy.stay.bed.number}` : "—"}</p>
        <p>Qarzdorlik: {formatMoney(c.debt)}</p>
      </section>
      <section className="card p-5">
        <h2 className="font-semibold">Yashash / kirish-chiqish</h2>
        {c.stays.map((s) => (
          <p key={s.id} className="mt-2 text-sm">{s.room.number}/{s.bed.number} · {formatDate(s.startDate)} · {s.status} · {formatMoney(s.paidAmount)}/{formatMoney(s.totalAmount)}</p>
        ))}
      </section>
      <section className="card p-5">
